...
async def retry_messages_job():
    ...
    while await PendingMessage.collection.count_documents(find_params):
        async for pending in PendingMessage.collection \
                .find(find_params).sort([('message.time', 1)]).batch_size(256):
            ...
            if j >= 20000:
                # await join_pending_message_tasks(tasks, actions_list=actions,
                #                                  messages_actions_list=messages_actions)
                gtasks.append(
                    join_pending_message_tasks(tasks, actions_list=actions,
                                               messages_actions_list=messages_actions))
                tasks = []
                actions = []
                messages_actions = []
...
async def handle_pending_tx(pending, actions_list):
    LOGGER.info("%s Handling TX in block %s"
    ...
    if messages is not None:
        ...
        actions_list.append(DeleteOne({'_id': pending['_id']}))
    # LOGGER.info("%s Handled TX in block %s"
    #             % (pending['context']['chain_name'], pending['context']['height']))
